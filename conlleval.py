...
def get_result(correct_chunks, true_chunks, pred_chunks,
    ...
    df = pd.DataFrame(columns=['Tag Type', 'Precision', 'Recall', 'F1', 'num_correct', 'num_predictions', 'num_entities'])
    ...
    for t in chunk_types:
        ...
        df = df.append({
        'Tag Type': t,
        'Precision': prec,  # Convert to percentage
        'Recall': rec,      # Convert to percentage
        'F1': f1,
        'num_correct': correct_chunks[t],
        'num_predictions': pred_chunks[t],
        'num_entities': true_chunks[t]
    }, ignore_index=True)
...
def evaluate(true_seqs, pred_seqs, verbose=True):
    ...
    result, df = get_result(correct_chunks, true_chunks, pred_chunks,
